...
- 현재 유저 정보를 받아올 수 있게 커스텀 데코레이터인 user.decorator를 만들었습니다.
  ![CurrentUser](https://user-images.githubusercontent.com/97301076/199891019-bc490e12-c8c7-476f-9ecc-f0650828378b.png)
- 이후 Task(일감)을 생성할 때 현재 유저의 정보를 같이 넘겨주어서 Task(일감)에 있는 유저 정보에 해당 값이 들어갈 수 있도록 구현했습니다.
- Task(일감)을 조회하고 수정하고 삭제하는 경우에는 작성된 Task(일감)의 유저 정보와 현재 유저의 정보가 일치하는 경우에만 접근이 가능하게 구현했습니다.<br>

### 3) Entity

- 해당 유저 삭제 시 해당 일감도 같이 삭제될 수 있도록 작성했습니다.
...
- Entity + DTO ( Data Transfer Object ) <br>
  ...
  ![Dto](https://user-images.githubusercontent.com/97301076/199895517-c0caeee3-0df6-4e0a-963a-94355b5081f0.png)<br>

### 4) Swagger API - 보안 처리

- Swagger 내부 정보가 공개적으로 노출되지 않도록 보안 처리를 해주었습니다.
...
- 해당 테스트가 종료되면 Synchronize를 통해 DB를 초기화해 주었습니다.
  ![test_db_초기화](https://user-images.githubusercontent.com/97301076/199909453-5da5481b-51e4-4c5f-9506-66785d6dc55d.png)
...
그래도 어떠한 방향성을 가지고 공부를 해야 할지는 알게 되어서 좋습니다.

이번 프로젝트가 끝나갈 때쯤 테스트 코드의 중요성을 알게 되어서 뒤늦게 E2E TEST라도 진행했는데 앞으로는 하나하나의 기능을 구현하면서 같이 Unit Test도 작성해 보고 싶습니다.

평소에 지식을 습득한 후에 구현하는 과정에 익숙해져 있었는데 이번에는 필요한 것들을 공부해가면서 적용하다 보니 더욱더 많이 배울 수 있었습니다.

최근에는 공부를 하면 할수록 배워야 할 게 끝도 없이 늘어나는 느낌이라 조금은 힘들었는데, 구직 전에 간단한 프로젝트를 진행하면서 기본기를 다지고 가는 느낌이라 힘이 납니다.
